...
namespace Day2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            impl.Watch = System.Console.WriteLine;
            impl.Execute();
            Console.WriteLine($"Result : {impl.Result}");

            for(int i=0;i<10000;i++)
            {
                impl=new IntCodeComputer(program,i);
                impl.Execute();
                if (impl.Result==19690720) {
                    Console.WriteLine($"State for result {impl.Result} : {impl.State}");
                    break;
                }
            }
