...
namespace Day23
{
    ...
    class NetworkSwitch
    {
        private ConcurrentDictionary<long,ComputerWithNic> Computers=new ConcurrentDictionary<long, ComputerWithNic>();
        private long nextAddress=0;
        public long Plug(ComputerWithNic computer)
        {
            long address=nextAddress++;
            if (!Computers.TryAdd(address,computer)) throw new Exception("Could not plug computer !");
            return address;
        }

        private BlockingCollection<Message> buffer=new BlockingCollection<Message>(new ConcurrentQueue<Message>());
        ...
        private Message NATMessage {get;set;}
        private Message PreviousNATMessage {get;set;}
        
    }
    class ComputerWithNic
    {
        public IntCodeComputer Computer {get;set;}
        public long Address { get; }
        public NetworkSwitch NetworkSwitch { get; }

        public ComputerWithNic(string program, NetworkSwitch networkSwitch)
        {
            Computer=new IntCodeComputer(program);
            Computer.GetNextInput=GetNextInput;
            Computer.SetOutput=SetOutput;
            NetworkSwitch = networkSwitch;
            Address = NetworkSwitch.Plug(this);
        }

        ConcurrentQueue<Message> InputQueue=new ConcurrentQueue<Message>();
        public void Receive(Message msg) => InputQueue.Enqueue(msg);

        private int inputStatus=0;
        private Message receivingMessage=null;
        private int TryDequeueCount {get;set;}=0;
        public bool Idle => TryDequeueCount>1;
        private long GetNextInput()
        {
            switch(inputStatus)
            {
                case 0: inputStatus=1; return Address;
                case 1: 
                    if (InputQueue.TryDequeue(out receivingMessage))
                    {
                        inputStatus=2;
                        TryDequeueCount=0;
                        return receivingMessage.X;
                    } else { 
                        Thread.Sleep(10);
                        TryDequeueCount++;
                        return -1;
                    }
                case 2: inputStatus=1; return receivingMessage.Y;
        ...
        private void SetOutput(long? output)
        {
            ...
            {
                case 0: 
                    sendingMessage=new Message { Address=output.Value, From=Address };
                    outputStatus=1;
                    break;
                case 1:
                    sendingMessage.X=output.Value;
                    outputStatus=2;
                    break;
                case 2:
                    sendingMessage.Y=output.Value;
                    NetworkSwitch.Send(sendingMessage);
                    sendingMessage=null;
                    outputStatus=0;
                    TryDequeueCount=0;
                    break;
        ...
        static TaskFactory taskFactory=new TaskFactory(TaskCreationOptions.LongRunning, TaskContinuationOptions.LongRunning);

        public Task Run()
        {
            
            return taskFactory.StartNew(()=>{
                Console.WriteLine($"Computer {Address} started");
                Computer.Execute();
                Console.WriteLine($"Computer {Address} stopped");
            });
        }
    }
