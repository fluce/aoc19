...
namespace Day25
{
    public class Node
    {
        ...
        public Dictionary<string,Node> Neighbours {get;set;} = new Dictionary<string, Node>();
        public HashSet<string> ItemsOriginalyHere {get;set;} = new HashSet<string>();
        public bool HasSensitivePlate {get;set;}
    }
    class InteractiveComputer
    {
        ...
        public InteractiveComputer(string program)
        {
            Computer=new IntCodeComputer(program);
            Computer.SetOutput=Output;
            Computer.GetNextInput=GetNextInput;
        }

        private StringBuilder inputBuffer=new StringBuilder();
        private string lastCommand;

        private Queue<char> nextCommands = new Queue<char>();
     
        public void EnqueueCommand(string command) {
            Console.WriteLine($"Queueing {command}");
            foreach(var c in command) nextCommands.Enqueue(c);
            nextCommands.Enqueue(Convert.ToChar(10));
        }

        public long GetNextInput()
        {
            if (nextCommands.Count>0) {
                var c=nextCommands.Dequeue();
                if (c==10) { lastCommand=inputBuffer.ToString(); inputBuffer.Clear(); Console.WriteLine($"Queued command {lastCommand}");}
                else inputBuffer.Append(c);
                return c;
            } else {
                var a=Console.ReadKey();
                if (a.Key==ConsoleKey.Enter) { lastCommand=inputBuffer.ToString(); inputBuffer.Clear(); Console.WriteLine(); if (lastCommand=="map") DumpMap(); return 10; }
                inputBuffer.Append(a.KeyChar);
                return a.KeyChar;
            }
        }
        private StringBuilder builder=new StringBuilder();
        private Node currentlyBuildingNode;

        private void DumpNode(Node s)
        {
            Console.WriteLine($"{s.Name}");
            foreach(var direction in s.Neighbours)
            {
                Console.WriteLine($"  {direction.Key} => {direction.Value?.Name}");
            }
        }
        ...
        LinkedList<Node> VisitedRooms=new LinkedList<Node>();
        Stack<Node> RetracePath=new Stack<Node>();

        string[] ForbiddenItems={ "infinite loop", "molten lava", "photons", "giant electromagnet", "escape pod"};
        string commandToQueue=null;
        
        List<Node> PathToSecurityRoom=null;

        int globalStatus=0;

        SortedSet<string> TriedTooLight=new SortedSet<string>();
        SortedSet<string> TriedTooHeavy=new SortedSet<string>();
        Queue<string[]> ToTry=new Queue<string[]>();
        List<string> Inventory=new List<string>();
        bool readingInventory=false;

        string[] tryingCombination;
        string joinedTryingCombination;
        ...
        public void Output(long? output)
        {
            if (output.Value==10) {
                ...
                if (s.StartsWith("==")) {
                    commandToQueue=null;
                    var name=Regex.Match(s,"== (?<a>.*) ==").Groups["a"]?.Value;
                    if (Map.ContainsKey(name)) { 
                        Console.WriteLine("Already been here");  
                        if (VisitedRooms.Count>0 && lastCommand!=null && VisitedRooms.First.Value.Neighbours[lastCommand]==null)
                        {
                            Console.WriteLine($"Connecting room {VisitedRooms.First.Value.Name} to {Map[name].Name} ({lastCommand})");
                            VisitedRooms.First.Value.Neighbours[lastCommand]=Map[name];                        
                        }

                        VisitedRooms.AddFirst(Map[name]);
                        currentlyBuildingNode=null;
                    }
                    else {
                        currentlyBuildingNode=new Node();
                        currentlyBuildingNode.Name=name;
                        Map[currentlyBuildingNode.Name]=currentlyBuildingNode;
                        if (VisitedRooms.Count>0 && VisitedRooms.First.Value.Neighbours[lastCommand]==null)
                        {
                            Console.WriteLine($"Connecting room {VisitedRooms.First.Value.Name} to {currentlyBuildingNode.Name} ({lastCommand})");
                            VisitedRooms.First.Value.Neighbours[lastCommand]=currentlyBuildingNode;                                                    
                        }
                        VisitedRooms.AddFirst(currentlyBuildingNode);
                    }
                } 
                if (currentlyBuildingNode!=null) {
                    if (s=="Items here:") status=2;
                    if (status==2 && s.StartsWith("- ")) {
                        var item=Regex.Match(s,"- (?<a>.*)").Groups["a"].Value;
                        if (!ForbiddenItems.Contains(item)) {
                            Console.WriteLine($" item {item} {commandToQueue==null}");
                            if (commandToQueue==null)
                                commandToQueue=$"take {item}";
                        }
                        currentlyBuildingNode.ItemsOriginalyHere.Add(item);
                    }
                    if (s=="Doors here lead:") status=1;
                    if (status==1 && s.StartsWith("- ")) {
                        var dir=Regex.Match(s,"- (?<a>.*)").Groups["a"].Value;
                        //Console.WriteLine($"dir={dir} lastCommand={lastCommand}");
                        if (   (dir=="east" && lastCommand=="west") || (dir=="west" && lastCommand=="east") 
                            || (dir=="north" && lastCommand=="south") || (dir=="south" && lastCommand=="north") )
                            currentlyBuildingNode.Neighbours[dir]=VisitedRooms.First.Next?.Value;
                        else {
                            currentlyBuildingNode.Neighbours[dir]=null;
                            Console.WriteLine($"You should visit there ! {dir}");
                        }
                    }
                }
                if (s=="Items in your inventory:")  { readingInventory=true; Inventory.Clear(); }
                else
                if (readingInventory) {
                    if (s.StartsWith("- ")) {
                        var item=Regex.Match(s,"- (?<a>.*)").Groups["a"].Value;
                ...
                if (s.StartsWith("A loud")) { 
                    if (globalStatus==3)
                    {
                        if (s.Contains("heavier")) // too light
                        {
                            TriedTooLight.Add(joinedTryingCombination);
                            foreach(var a in tryingCombination)
                                EnqueueCommand($"drop {a}");
                        }
                        if (s.Contains("lighter")) // too heavy
                        {
                            TriedTooHeavy.Add(joinedTryingCombination);
                            foreach(var a in tryingCombination)
                                EnqueueCommand($"drop {a}");
                        }
                    } else {
                        VisitedRooms.First.Value.HasSensitivePlate=true;
                        if (PathToSecurityRoom==null) PathToSecurityRoom=RetracePath.Reverse().ToList();
                        RetracePath.Pop();
                    }
                    lastCommand=null;
                }
                if (s.StartsWith("Command?")) {
                    var currentNode=VisitedRooms.First.Value;
                    DumpNode(currentNode);
                    if (globalStatus==0) {
                        if (commandToQueue!=null)
                            EnqueueCommand(commandToQueue);
                        else {
                            var roomToVisit=currentNode.Neighbours.FirstOrDefault(x=>x.Value==null);
                            if (roomToVisit.Key!=null && roomToVisit.Value==null) {
                                EnqueueCommand(roomToVisit.Key);
                                RetracePath.Push(currentNode);
                            } else if (RetracePath.Count>0) {
                                var prev=RetracePath.Pop();
                                var dir=currentNode.Neighbours.FirstOrDefault(x=>x.Value==prev).Key;
                                if (dir!=null) EnqueueCommand(dir);
                            } else {
                                Console.WriteLine("Path to security room :");
                                var i=PathToSecurityRoom.First();
                                foreach(var e in PathToSecurityRoom.Skip(1)) {
                                    var dir=i.Neighbours.First(x=>x.Value==e).Key;
                                    Console.WriteLine($" - {i.Name} => {dir} ({e.Name})");
                        ...
                        if (currentNode.Name=="Security Checkpoint")
                        {
                            Console.WriteLine($"Global Status={globalStatus}");
                            switch (globalStatus)
                            {
                                case 1: EnqueueCommand("inv"); globalStatus=2; break;
                                case 2: {
                                    foreach(var e in Inventory) EnqueueCommand($"drop {e}");
                                    foreach (var e in ListCombinations(Inventory.ToArray()))
                                        ToTry.Enqueue(e);
                                    Console.WriteLine($"Combinations to try : {ToTry.Count}");
                                    globalStatus=3;
                                    break;
                                }
                                case 3: {
                                    if (!nextCommands.Any())
                                    {
                                        Console.WriteLine($"ToTry queue : {ToTry.Count()} {TriedTooHeavy.Count()} {TriedTooLight.Count()}");
                                        string[] totry=null;
                                        string joinedToTry=null;
                                        while (totry==null) {
                                            totry=ToTry.Dequeue();
                                            joinedToTry=string.Join(",",totry);
                                            if (TriedTooHeavy.Any(x=>joinedToTry.StartsWith(x))) // already tried
                                                totry=null;
                                            else if (TriedTooLight.Contains(joinedToTry))
                                                totry=null;
                                        }
                                        if (totry!=null) 
                                        {
                                            tryingCombination=totry;
                                            joinedTryingCombination=joinedToTry;
                                            foreach(var item in totry) {
                                                EnqueueCommand($"take {item}");
                                            }
                                            EnqueueCommand(currentNode.Neighbours.First(x=>x.Value.HasSensitivePlate).Key);
                                            //Console.ReadKey();
                                        } else
                                            globalStatus=4;
                                    }
                ...
                builder.Append(Convert.ToChar(output.Value));
        }
        ...
        public IEnumerable<string[]> ListCombinations(IEnumerable<string> items)
        {
            foreach(var first in items) {
                var prefix=new string[] { first };;
                yield return prefix;
                var others=items.Where(x=>x!=first).ToArray();
                if (others.Length>0)
                    foreach(var combination in ListCombinations(others))
                    {
                        yield return prefix.Concat(combination).OrderBy(x=>x).ToArray();
                    }
            }
        }
    }
