...
namespace Day3
{
    ...
    {        
        ...
        public static implicit operator Point((int X,int Y) p) => new Point() { X=p.X, Y=p.Y };
        public override string ToString()=>$"({X},{Y})";
    };

    public class WirePath
    {
        public WirePath(string path)
        {
            Path = path;
        }
        ...
        {
            private PathElement(string element)
            {
                ...
                Distance=int.Parse(element.Substring(1));
            }
            public Point Direction {get;}
            public int Distance {get;}

            public static PathElement Parse(string element)=>new PathElement(element);
        }

        IEnumerable<PathElement> BuildPathElement()
        {
            return Path.Split(',').Select(PathElement.Parse);
        }

        public IEnumerable<Point> BuildStepByStepPath()
        {
            Point current=(0,0);
            foreach(var pathElement in BuildPathElement())
            {
                foreach(var step in Enumerable.Range(1,pathElement.Distance)) {
                    current.X+=pathElement.Direction.X;
                    current.Y+=pathElement.Direction.Y;
    ...
    public class Day3Impl
    {
        public Day3Impl(string input1, string input2)
        {
            Input1 = input1;
            Input2 = input2;
        }
        ...
        private string GetDisplay(IEnumerable<Point> path1, IEnumerable<Point> path2)
        {
            Point a1=(path1.Min(x=>x.X),path1.Min(x=>x.Y));
            Point b1=(path1.Max(x=>x.X),path1.Max(x=>x.Y));
            Point a2=(path2.Min(x=>x.X),path2.Min(x=>x.Y));
            Point b2=(path2.Max(x=>x.X),path2.Max(x=>x.Y));
            Point A=(Math.Min(a1.X,a2.X)-1,Math.Min(a1.Y,a2.Y)-1);
            Point B=(Math.Max(b1.X,b2.X)+1,Math.Max(b1.Y,b2.Y)+1);

            Point size=(B.X-A.X,B.Y-A.Y);
            char[][] display=Enumerable.Range(0,size.Y).Select(x=>(new string(' ',size.X)).ToCharArray()).ToArray();
            display[0-A.Y][0-A.X]='0' ;
            foreach (var p in path1) display[p.Y-A.Y][p.X-A.X]='o' ;
            foreach (var p in path2) display[p.Y-A.Y][p.X-A.X]=display[p.Y-A.Y][p.X-A.X]=='o'?'x':'·';

            return string.Join('\n',display.Select(x=>new string(x)));
        }

        public void Execute()
        {
            var path1=new WirePath(Input1).BuildStepByStepPath().Zip(Enumerable.Range(1,int.MaxValue),(p,i)=>(id:1,idx:i,p)).ToList();
            var path2=new WirePath(Input2).BuildStepByStepPath().Zip(Enumerable.Range(1,int.MaxValue),(p,i)=>(id:2,idx:i,p)).ToList();

            var comparer=new LoggingComparer();
            path1.Intersect(path2,comparer).ToList();
            Intersections = comparer.Log;

            ResultMinDistance=Intersections.Min(x=>x.Item1.p.ManhattanDistanceToOrigin);
            ResultMinSteps=Intersections.Min(x=>x.Item1.idx+x.Item2.idx);
            
            Display=GetDisplay(path1.Select(x=>x.p),path2.Select(x=>x.p));

        }

        public int ResultMinDistance {get;private set;}
        public int ResultMinSteps {get;private set;}
        public List<((int id, int idx,Point p),(int id, int idx,Point p))> Intersections {get; private set;}

        public string Display { get;set;}
    }

    class LoggingComparer: IEqualityComparer<(int id, int idx,Point p)>    
    {
        public List<((int id, int idx,Point p),(int id, int idx,Point p))> Log {get;}=new List<((int id, int idx, Point p), (int id, int idx, Point p))>();

        public bool Equals((int id, int idx,Point p) p1, (int id, int idx,Point p) p2) 
        {
            var ret=p1.p.Equals(p2.p) && p1.id!=p2.id;
            if (ret)
                Log.Add((p1,p2));
            return ret;
        }
        public int GetHashCode((int id, int idx, Point p) p) 
        {
            return p.p.GetHashCode();
        }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            impl.Execute();
            if (impl.Display.Length<4000) Console.WriteLine(impl.Display);
            foreach(var i in impl.Intersections)
            {
                Console.WriteLine($"{i.Item1.id} {i.Item1.idx} : {i.Item1.p} / {i.Item2.id} {i.Item2.idx} : {i.Item2.p}");
            }
            Console.WriteLine($"Min Distance: {impl.ResultMinDistance}");
            Console.WriteLine($"Min Step: {impl.ResultMinSteps}");
